...
const DishesCards = ({ data: { info, restaurant: { info: resInfo }, hideRestaurantDetails = false } }) => {
    ...
    return (
        <>
            <div className={`h-screen w-full absolute top-0 left-0 z-40  ${toggleCard === info?.id ? "visible" : "invisible"}`}>
                <div
                    onClick={() => toggleDetails()}
                    className={`h-full w-full bg-[#050a13db] opacity-80 `}>
                </div>
                <div className="bg-white w-4/5 md:w-1/3 rounded-2xl shadow-lg absolute top-1/2 -translate-y-1/2  left-1/2 -translate-x-1/2 overflow-hidden">
                    <div className='relative'>
                        <img className='w-full aspect-[1.2] object-cover' src={`https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,h_600/${imageId}`} alt="" />
                        <div
                            onClick={() => toggleDetails()}
            ...
            <div className=' bg-white rounded-2xl px-4 pt-5'>
                ...
                <div className='flex justify-between mt-3'>
                    ...
                    {imageId ?
                        <div className='w-32 sm:w-40  overflow-hidden'>
                            <img className='object-cover rounded-xl aspect-1 w-full' src={`https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_208,h_208,c_fit/${imageId}`} alt="" />
                            <div

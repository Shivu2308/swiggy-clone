...
const RasturantCards = ({ data: { info } }) => {
    ...
    return (
        <Link to={`/city/${city}/${restaurant}-${"rest" + id}`}>
            <div className='bg-white px-4 pt-5 pb-10 m-2 cursor-pointer flex items-center gap-4 flex-shrink-0'>
                <div className='relative w-24'>
                    ...
                    <div className='w-full aspect-[11/12] rounded-lg overflow-hidden'>
                        <img className='w-full h-full object-cover' src={`https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_264,h_288,c_fill/${cloudinaryImageId}`} alt="" />
                    </div>
                    {info?.aggregatedDiscountInfoV3?.header &&
                        <div className='absolute left-1/2 -bottom-2 -translate-x-1/2 text-center'>
                            {info?.aggregatedDiscountInfoV3?.discountTag &&
                                <span className='bg-[#FF5200] text-white text-[9px] font-extrabold leading-none px-1 py-1 rounded-t-xl'>
                                    {info?.aggregatedDiscountInfoV3?.discountTag}
                                </span>}
                            ...
                            <div className='text-[#FF5200] flex items-center justify-center text-center flex-col bg-white w-fit rounded-lg px-2 py-[6px] shadow-lg '>
                                <span className='font-bold leading-none text-sm tracking-tighter min-w-[60px]'>{info?.aggregatedDiscountInfoV3?.header}</span>
                                <span className='text-[8px] leading-none font-bold tracking-tighter '>• {info?.aggregatedDiscountInfoV3?.subHeader} •</span>
                            </div>
                        </div>
                    }
                </div>
                ...
                <div>
                    ...
                    <div className='text-[13px] font-bold flex items-center gap-1 text-[#7c7d7f] '>
